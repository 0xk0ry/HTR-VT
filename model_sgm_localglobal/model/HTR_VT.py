...
# ---- 1-D windowed attention (width axis) ----


class WindowMHSA1D(nn.Module):
    """
    1-D windowed MHSA over sequence [B, N, D]. Optional Swin-style shift.
    ...
    def forward(self, x):  # [B, N, D]
        B, N, D = x.shape
        w = self.win
        s = self.shift % w if w > 0 else 0

        # Shift (roll) for shifted windows
        if s:
        ...
        # chunk into windows: [B, nW, w, D] -> [B*nW, w, D]
        ...
        # stitch back, unpad, unshift
        out = out.reshape(B, nW, w, D).reshape(B, Np, D)
        if pad:
            out = out[:, :N, :]
        if s:
            out = torch.roll(out, shifts=-s, dims=1)
        return out


# ---- pooled global MHSA (fixed token budget) ----
class PooledGlobalMHSA(nn.Module):
    """
    Adaptive pool along width to fixed G tokens, run MHSA there, interpolate back to N.
    Keeps order → CTC safe. In/Out: [B, N, D].
    """
    ...
    def __init__(self, dim, num_heads, g_tokens=64, pool='avg',
        ...
        # small norm for stability on pooled tokens
        self.norm = nn.LayerNorm(dim, elementwise_affine=False)

    def forward(self, x):  # [B, N, D]
        B, N, D = x.shape
        G = min(self.g_tokens, max(1, N))

        # pool width tokens to G
        ch_first = x.transpose(1, 2)  # [B, D, N]
        if self.pool == 'avg':
            pooled = F.adaptive_avg_pool1d(ch_first, G)  # [B, D, G]
        else:
            pooled = F.adaptive_max_pool1d(ch_first, G)
        z = pooled.transpose(1, 2)     # [B, G, D]
        z = self.norm(z)

        # MHSA on G tokens
        qkv = self.qkv(z).reshape(B, G, 3, self.num_heads, D //
                                  self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = self.attn_drop(attn.softmax(dim=-1))
...
# ---- wrappers that keep your Block API/shape ----
class LocalBlock1D(nn.Module):
    """Pre-LN → WindowMHSA1D → +res → Pre-LN → MLP → +res"""

    def __init__(self, dim, num_heads, window, shift=False, mlp_ratio=4., qkv_bias=True,
...
class LocalGlobalParallelBlock(nn.Module):
    """Pre-LN → (Local || Pooled-Global) → Concat→Linear→ +res → Pre-LN→MLP→ +res"""

    def __init__(self, dim, num_heads, window, g_tokens=64, pool='avg',
                 mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim, elementwise_affine=True)
        self.local_attn = WindowMHSA1D(dim, num_heads, window, shift=0,
                                       qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x):
        y = self.norm1(x)
        yl = self.local_attn(y)
        yg = self.global_attn(y)
...
class GlobalPooledBlock(nn.Module):
    """Pre-LN → Pooled-Global MHSA → +res → Pre-LN→MLP→ +res"""
    ...
    def __init__(self, dim, num_heads, g_tokens=64, pool='avg',
        ...
        super().__init__()
        self.norm1 = norm_layer(dim, elementwise_affine=True)
        self.attn = PooledGlobalMHSA(dim, num_heads, g_tokens=g_tokens, pool=pool,
                                     qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        self.norm2 = norm_layer(dim, elementwise_affine=True)
        self.mlp = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio),
                       act_layer=act_layer, drop=drop)
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,
        ...
        window_w = 12   # try 8/12/16
        g_tokens = 64   # try 48/64/96 if lines vary a lot
        pool_type = 'avg'

        self.blocks = nn.ModuleList([
            LocalBlock1D(embed_dim, num_heads, window=window_w, shift=False,
                         mlp_ratio=mlp_ratio, qkv_bias=True, drop=0.0, attn_drop=0.0,
                         act_layer=nn.GELU, norm_layer=norm_layer),
            LocalBlock1D(embed_dim, num_heads, window=window_w, shift=True,
                         mlp_ratio=mlp_ratio, qkv_bias=True, drop=0.0, attn_drop=0.0,
                         act_layer=nn.GELU, norm_layer=norm_layer),
