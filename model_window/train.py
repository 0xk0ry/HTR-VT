...
def main():
    ...
    for nb_iter in range(start_iter, args.total_iter):
        ...
        image = batch[0].to(device='cuda', non_blocking=True)
        enc = converter.encode(batch[1])

        # pass 1
        with autocast('cuda'):                                     # mixed-precision forward
            loss = compute_loss(args, model, image, batch_size=image.size(0),
                                criterion=criterion, enc=enc)
        scaler.scale(loss).backward()
        scaler.unscale_(optimizer)                           # so SAM can read real grads
        optimizer.first_step(zero_grad=True)

        # pass 2
        with autocast('cuda'):
            loss2 = compute_loss(args, model, image, batch_size=image.size(0),
                                criterion=criterion, enc=enc)
