...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,
        ...
        # Additional tone head for per-frame tone predictions: NONE, ACUTE, GRAVE, HOOK, TILDE, DOT
        self.tone_head = nn.Linear(embed_dim, 6)
    ...
    def generate_span_mask(self, x, mask_ratio, max_span_length):
        ...
        total = int(L * mask_ratio)
        if total <= 0 or max_span_length <= 0:
            return mask
        num_spans = max(1, total // max_span_length)
        high = max(1, L - max_span_length + 1)
        for _ in range(num_spans):
            start = int(torch.randint(0, high, (1,), device=x.device).item())
            mask[:, start:start + max_span_length, :] = 0
        return mask
    ...
    def forward(self, x, mask_ratio=0.0, max_span_length=1, use_masking=False):
        ...
        # Normalize transformer features once, then branch into heads
        features = self.norm(x)
        # Base logits (B, T, |B|)
        base_logits = self.head(features)
        # Tone logits (B, T, 6), taken from the same pre-post-head-layernorm features
        tone_logits = self.tone_head(features)

        return base_logits, tone_logits
