...
def get_args_parser():
    ...
    parser.add_argument('--lambda-guard', default=0.0, type=float, help='Optional consonant-guard penalty weight')
    parser.add_argument('--tone-guard-weight', dest='lambda_guard', default=argparse.SUPPRESS, type=float,
                        help='Alias of --lambda-guard for consonant-guard penalty weight')
    # Inference margin for tone vs NONE
    parser.add_argument('--tone-kappa', default=0.2, type=float, help='Margin threshold over NONE to accept a tone at eval')
    parser.add_argument('--tone-margin-kappa', dest='tone_kappa', default=argparse.SUPPRESS, type=float,
                        help='Alias of --tone-kappa (eval margin vs NONE)')
    # Future augmentation toggle
    parser.add_argument('--tone-augment', action=argparse.BooleanOptionalAction, default=False,
                        help='Enable diacritic-centric augmentation (if available)')
