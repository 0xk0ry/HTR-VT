...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,
        ...
        # Multi-task heads
        self.base_head = nn.Linear(embed_dim, nb_cls)  # base char head
        self.diacritic_head = nn.Linear(embed_dim, 6)  # diacritic head
    ...
    def forward(self, x, mask_ratio=0.0, max_span_length=1, use_masking=False):
        ...
        x = self.norm(x)
        base_logits = self.base_head(x)  # (B, T, nb_cls)
        diacritic_logits = self.diacritic_head(x)  # (B, T, 6)
        return base_logits, diacritic_logits
