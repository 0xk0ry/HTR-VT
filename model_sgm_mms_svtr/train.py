...
def compute_losses(
    ...
    # 2) CTC loss with label smoothing to prevent overconfident predictions
    ...
    # Apply label smoothing by adding entropy regularization
    log_probs = preds.permute(1, 0, 2).log_softmax(2).float()
    loss_ctc = criterion_ctc(log_probs, text_ctc.cuda(), preds_sz, length_ctc.cuda()).mean()
    
    # Add entropy regularization to encourage diversity (label smoothing effect)
    if nb_iter > 500:  # Apply after initial training
        probs = torch.softmax(preds, dim=-1)
        entropy = -torch.sum(probs * torch.log(probs + 1e-8), dim=-1).mean()
        entropy_weight = 0.1  # Small weight for entropy regularization
        loss_ctc = loss_ctc - entropy_weight * entropy
...
def main():
    ...
    # Add learning rate warmup to prevent early collapse
    warmup_iters = 1000  # Warm up for first 1000 iterations
    base_lr = 1e-7
    ...
    for nb_iter in range(start_iter, args.total_iter):
        ...
        # Enhanced learning rate scheduling with additional warmup for stability
        if nb_iter < warmup_iters:
            # Extra linear warmup to prevent early collapse (on top of existing warmup)
            warmup_factor = min(1.0, (nb_iter + 1) / warmup_iters)
            for param_group in optimizer.param_groups:
                param_group['lr'] = base_lr * warmup_factor * 0.1  # Start with 10% of base LR
            current_lr = base_lr * warmup_factor * 0.1
        else:
            # Use existing cosine scheduling after enhanced warmup
            optimizer, current_lr = utils.update_lr_cos(
                nb_iter - warmup_iters, args.warm_up_iter, args.total_iter - warmup_iters, args.max_lr, optimizer)
        ...
        # Apply gradient clipping to prevent collapse
        if hasattr(model, 'gradient_clip_val'):
            torch.nn.utils.clip_grad_norm_(model.parameters(), model.gradient_clip_val)
        if sgm_head is not None:
            torch.nn.utils.clip_grad_norm_(sgm_head.parameters(), 1.0)
        ...
        # Apply gradient clipping to second pass as well
        if hasattr(model, 'gradient_clip_val'):
            torch.nn.utils.clip_grad_norm_(model.parameters(), model.gradient_clip_val)
        if sgm_head is not None:
            torch.nn.utils.clip_grad_norm_(sgm_head.parameters(), 1.0)
        ...
        if nb_iter % args.eval_iter == 0:
            ...
            with torch.no_grad():
                ...
                # Check for model collapse to blank predictions
                if val_cer >= 0.99 and val_wer >= 0.99 and nb_iter > 2000:
                    blank_check_batch = next(train_iter)
                    with torch.no_grad():
                        model_ema.ema.eval()
                        test_logits = model_ema.ema(blank_check_batch[0].cuda())
                        test_preds = test_logits.argmax(dim=-1)
                        blank_ratio = (test_preds == 0).float().mean().item()
                        
                    logger.warning(f"⚠️  Potential model collapse detected at iter {nb_iter}:")
                    logger.warning(f"   CER: {val_cer:.4f}, WER: {val_wer:.4f}, Blank ratio: {blank_ratio:.4f}")
                    
                    if blank_ratio > 0.95:
                        logger.warning("🚨 Model has collapsed to blank predictions! Consider:")
                        logger.warning("   1. Reducing learning rate further")
                        logger.warning("   2. Increasing anti-blank bias")
                        logger.warning("   3. Adding label smoothing")
                        logger.warning("   4. Using different initialization")
                        
                        # Optionally restore from a previous checkpoint
                        # break  # Uncomment to stop training on collapse
