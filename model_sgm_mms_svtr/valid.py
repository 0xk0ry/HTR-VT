...
def validation(model, criterion, evaluation_loader, converter):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        # Debug printing for first few validation samples
        if count <= 3:
            print(f"\n=== VALIDATION DEBUG SAMPLE {count} ===")
            print(f"Ground truth: '{labels[0] if labels else 'N/A'}'")
            print(f"Predicted: '{preds_str[0] if preds_str else 'N/A'}'")
            print(f"Preds shape after permute: {preds.shape}")
            print(f"Preds_size: {preds_size.tolist()}")
            print(f"Preds_index shape: {preds_index.shape}")
            print(f"Preds_index sample (first 30): {preds_index[:30].tolist()}")
            print(f"CTC decoded strings: {preds_str}")
