...
class TransformerDecoder(nn.Module):
    ...
    def greedy_decode(self, memory, sos_token_id, eos_token_id, max_len=None, memory_key_padding_mask=None, temperature=1.0, repetition_penalty=1.1):
        ...
        decoded = torch.full((1, batch_size), sos_token_id,
                        ...
                        for token in unique_tokens:
                            if token != sos_token_id:  # Don't penalize SOS token
                                if logits_temp[batch_idx, token] > 0:
                                    logits_temp[batch_idx,
                                                token] /= repetition_penalty
                                else:
                                    logits_temp[batch_idx,
                                                token] *= repetition_penalty
    ...
    def nucleus_decode(self, memory, sos_token_id, eos_token_id, max_len=None, memory_key_padding_mask=None, temperature=0.8, top_p=0.9, repetition_penalty=1.1):
        ...
        for step in range(max_len - 1):
            ...
            next_token = self.nucleus_sampling(
                current_logits, top_p=top_p, temperature=temperature)
    ...
    def beam_search_decode(self, memory, sos_token_id, eos_token_id, beam_size=5, max_len=None, memory_key_padding_mask=None):
        ...
        assert memory.size(
            1) == 1, "Beam search currently supports batch size 1"
        ...
        if memory_key_padding_mask is not None:
            memory_key_padding_mask = memory_key_padding_mask.repeat(
                1, beam_size)

        # Initialize beams
        sequences = torch.full((beam_size, 1), sos_token_id,
                               dtype=torch.long, device=device)
        ...
        for step in range(max_len - 1):
            ...
            logits = self.decode_step(
                memory, decoder_input, memory_key_padding_mask)

            # Get log probabilities for next token: [beam_size, vocab_size]
            log_probs = F.log_softmax(logits[-1], dim=-1)

            # Compute scores for all possible next tokens
            candidate_scores = scores.unsqueeze(
                1) + log_probs  # [beam_size, vocab_size]
            ...
            for seq in new_sequences:
                if len(seq) < max_seq_len:
                    padded = torch.cat(
                        [seq, torch.zeros(max_seq_len - len(seq), dtype=torch.long, device=device)])
                else:
                    padded = seq
        ...
        if all_sequences:
            ...
            for i in sorted_indices[:beam_size]:
                ...
                if len(seq) < max_len_final:
                    padded = torch.cat(
                        [seq, torch.zeros(max_len_final - len(seq), dtype=torch.long, device=device)])
                else:
                    padded = seq
