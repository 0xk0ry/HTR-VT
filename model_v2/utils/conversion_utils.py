...
def create_encoder_decoder_tokenizer_from_ctc(ctc_converter):
    ...
    """
    ed_alphabet = convert_ctc_to_encoder_decoder_alphabet(ctc_converter.character)
...
def validate_tokenizer_compatibility(tokenizer1, tokenizer2):
    ...
    if tokenizer1.vocab_size != tokenizer2.vocab_size:
        differences.append(f"Vocab sizes differ: {tokenizer1.vocab_size} vs {tokenizer2.vocab_size}")
    ...
    if hasattr(tokenizer1, 'character') and hasattr(tokenizer2, 'character'):
        ...
        if chars1 != chars2:
            ...
            if only_in_1:
                differences.append(f"Characters only in tokenizer1: {only_in_1}")
            if only_in_2:
                differences.append(f"Characters only in tokenizer2: {only_in_2}")
...
def compare_model_outputs(ctc_model, ed_model, sample_input, ctc_converter, ed_tokenizer):
    ...
    with torch.no_grad():
        ...
        ctc_texts = ctc_converter.decode(ctc_preds[0].cpu(), [ctc_preds.shape[1]])
