...
def validation(model, criterion, evaluation_loader, converter, args=None):
    """
    Run validation or evaluation loop.
    Args:
        model: torch.nn.Module, the model to evaluate
        criterion: loss function (CTC)
        evaluation_loader: DataLoader, validation data
        converter: label converter (CTCLabelConverter or DualLabelConverter)
    Returns:
        val_loss: float, average validation loss
        CER/ WER: float, character/word error rate
        final_preds: list, predictions (composed if dual-head)
            ...
            for b in range(batch_size):
                ...
                for cls_idx, t_idx in segments:
                    ...
                    if ch:
                        ...
                        if args is not None and getattr(args, 'vowel_only_tags', False) and not vn_tags.is_vowel(ch):
                            # Force defaults for consonants
                            mod_ids.append(0)
                            tone_ids.append(0)
                        else:
