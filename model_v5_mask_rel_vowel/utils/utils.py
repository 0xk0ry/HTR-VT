...
def build_base_charset() -> str:
    """
    Return the base charset string (no precomposed vowels).
    """
...
def build_full_charset() -> str:
    """
    Return the full Vietnamese charset string (includes precomposed vowels).
    """
...
def get_logger(out_dir):
    """
    Create a logger that logs to both file and stdout.
    Args:
...
def update_lr_cos(nb_iter, warm_up_iter, total_iter, max_lr, optimizer, min_lr=1e-7):
    ...
    Args:
        nb_iter: int, current iteration
        warm_up_iter: int, warmup iterations
...
class DualLabelConverter(object):
    """Dual label converter for base characters + modifier/tone tags.

    encode(labels: List[str]) ->
        text_base: IntTensor concatenated indices over base charset (CTC targets)
        length_base: IntTensor of per-sample base lengths U
        tags_mod: List[LongTensor] per sample (length U) of modifier ids
        tags_tone: List[LongTensor] per sample (length U) of tone ids
        per_sample_U: IntTensor same as length_base (for convenience)
    """
    ...
    def encode(self, text: List[str]) -> Tuple[torch.IntTensor, torch.IntTensor, List[torch.LongTensor], List[torch.LongTensor], torch.IntTensor]:
        # Decompose each label into base + tags using vn_tags
        base_seqs: List[str] = []
        tags_mod: List[torch.LongTensor] = []
        tags_tone: List[torch.LongTensor] = []
        lengths: List[int] = []
        ...
        # Flatten base strings into indices
        flat = ''.join(base_seqs)
        try:
            ...
            missing = str(e)
            raise KeyError(
                f"Character {missing} not in base charset. Consider overriding --base-charset or updating build_base_charset().")
        ...
        return text_base_tensor, length_base_tensor, tags_mod, tags_tone, per_sample_U

    def decode(self, text_index: torch.Tensor, length: torch.Tensor) -> List[str]:
        # Same decoding as CTCLabelConverter for the base sequence.
        texts: List[str] = []
        index = 0
        for l in length:
            l = int(l)
