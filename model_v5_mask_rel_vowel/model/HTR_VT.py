...
class Attention(nn.Module):
    """
    Multi-head self-attention with learnable 1D relative positional bias.
    """
    def __init__(self, dim, num_patches, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        # (2L-1, heads) relative position bias table
...
def get_2d_sincos_pos_embed(embed_dim, grid_size):
    ...
    Returns:
        pos_embed: np.ndarray, shape [grid_size*grid_size, embed_dim]
    """
...
class LayerNorm(nn.Module):
    """Custom LayerNorm for input normalization."""
    def forward(self, x):
...
class MaskedAutoencoderViT(nn.Module):
    ...
    Supports dual-head output for Vietnamese diacritics.
    """
    ...
    def __init__(self,
        ...
        if self.use_dual_head:
            self.head_mod = torch.nn.Linear(embed_dim, 4)
            self.head_tone = torch.nn.Linear(embed_dim, 6)
    ...
    def initialize_weights(self):
        ...
        # initialize relative positional bias tables
    ...
    def generate_span_mask(self, x, mask_ratio, max_span_length):
        """Generate random span mask for input sequence."""
    ...
    def forward(self, x, mask_ratio=0.0, max_span_length=1, use_masking=False):
        """Forward pass for MaskedAutoencoderViT."""
        # embed patches
        x = self.layer_norm(x)
    ...
    # apply Transformer blocks (relative positional bias inside Attention)
        for blk in self.blocks:
            x = blk(x)
...
def create_model(nb_cls, img_size, **kwargs):
    """Factory for MaskedAutoencoderViT with default hyperparameters."""
