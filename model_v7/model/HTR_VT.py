...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_dual_head:
            # Lightweight additional heads
            self.head_mod = nn.Linear(embed_dim, mod_classes)
            self.head_tone = nn.Linear(embed_dim, tone_classes)
    ...
    def forward(self, x, mask_ratio=0.0, max_span_length=1, use_masking=False):
        ...
        # Tag heads share same encoded sequence (no extra norm to save compute)
        mod_logits = self.head_mod(x)
        tone_logits = self.head_tone(x)
        return {
            'base': base_logits,
            'mod': mod_logits,
            'tone': tone_logits
