...
def get_args_parser():
    ...
    # Dual-head & Vietnamese tag options
    parser.add_argument('--use-dual-head', action='store_true', default=False,
                        help='Enable dual-head prediction (base + modifier + tone)')
    parser.add_argument('--lambda-mod', type=float, default=1.0,
                        help='Loss weight for modifier head')
    parser.add_argument('--lambda-tone', type=float, default=1.0,
                        help='Loss weight for tone head')
    parser.add_argument('--tag-alpha', type=float, default=0.2,
                        help='Relative weight for consonant tag losses (when not vowel-only)')
    parser.add_argument('--vowel-only-tags', action='store_true', default=False,
                        help='If set, only supervise modifier/tone tags on vowels (consonant tags ignored)')
