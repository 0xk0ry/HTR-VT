...
class RobertaTextCorrector:
    ...
    def correct_text_contextual(self, text, vocab=None, max_iterations=2):
        """
        Contextual correction: mask and correct only non-vocab words, use full sentence context
        """
        ...
        # Mask non-vocab words
        if vocab is not None:
            masked_words = [w if w in vocab else self.mask_token for w in words]
        else:
            masked_words = words
        masked_text = " ".join(masked_words)
        ...
        # Replace masked tokens with high-confidence predictions
        input_ids = inputs['input_ids'][0]
        mask_indices = (input_ids == self.mask_token_id).nonzero(as_tuple=True)[0]
        corrected_ids = input_ids.clone()
        for idx in mask_indices:
            token_logits = logits[0, idx, :]
            probs = F.softmax(token_logits, dim=-1)
            top_token = torch.argmax(probs).item()
            confidence = probs[top_token].item()
            if confidence >= self.confidence_threshold:
                corrected_ids[idx] = top_token
        corrected_text = self.tokenizer.decode(corrected_ids, skip_special_tokens=True)
    ...
    def correct_batch_contextual(self, texts, vocab=None):
        """
        Correct a batch of texts using contextual correction
        """
        corrected_texts = []
        for text in texts:
            corrected = self.correct_text_contextual(text, vocab=vocab)
...
def validation_with_llm_correction(model, criterion, evaluation_loader, converter, text_corrector):
    ...
    vocab = set()
    try:
        with open(getattr(evaluation_loader.dataset, 'vocab_path', ''), 'r', encoding='utf-8') as vf:
            for line in vf:
                vocab.add(line.strip())
    except Exception:
        # fallback: use ralph values if available
        if hasattr(evaluation_loader.dataset, 'ralph'):
            vocab = set(evaluation_loader.dataset.ralph.values())

    def simple_ctc_beam_search(log_probs, beam_size=5):
        """
        Simple CTC beam search decoder for a single sample (log_probs: [T, C])
        Returns n-best decoded strings
        """
        # This is a simplified version; for production use, consider using a library like ctcdecode
        T, C = log_probs.shape
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        with torch.no_grad():
            ...
            # n-best decoding (beam search)
            preds_str = []
            best_llm_str = []
            for b in range(batch_size):
                log_probs = preds_log[:, b, :]
                nbest = simple_ctc_beam_search(log_probs, beam_size=5)
                candidates = [converter.decode(np.array(seq), np.array([len(seq)])) for seq, _ in nbest]
                # LLM rescoring
                llm_scores = [text_corrector.score_text(cand[0]) if cand else float('-inf') for cand in candidates]
                best_idx = int(np.argmax(llm_scores)) if llm_scores else 0
                preds_str.append(candidates[0][0] if candidates and candidates[0] else "")  # top-1 CTC
                best_llm_str.append(candidates[best_idx][0] if candidates and candidates[best_idx] else "")

        valid_loss += cost.item()
        count += 1

        # Use LLM-rescored best candidate as corrected prediction
        corrected_preds = best_llm_str
...
def main():
    ...
    # Lower confidence threshold for stricter corrections
    text_corrector = RobertaTextCorrector(
        model_name=getattr(args, 'roberta_model_name', 'roberta-large'),
        device=device,
        confidence_threshold=getattr(args, 'roberta_confidence_threshold', 0.7)
    )
    ...
    logger.info(
        f'Test loss: {val_loss:0.3f} \t CER: {cer_original:0.4f} \t WER: {wer_original:0.4f}')
    logger.info('=' * 80)
    logger.info('ROBERTA-CORRECTED RESULTS:')
    logger.info(
        f'Test loss: {val_loss:0.3f} \t CER: {cer_corrected:0.4f} \t WER: {wer_corrected:0.4f}')
    logger.info('=' * 80)
    logger.info('IMPROVEMENT:')
    cer_improvement = ((cer_original - cer_corrected) /
                       cer_original) * 100 if cer_original > 0 else 0
    wer_improvement = ((wer_original - wer_corrected) /
                       wer_original) * 100 if wer_original > 0 else 0
    ...
    with open(sample_file, 'w', encoding='utf-8') as f:
        ...
        for i in range(min(20, len(labels))):  # Show first 20 samples
            ...
            orig_cer = editdistance.eval(
                preds_original[i], labels[i]) / max(len(labels[i]), 1)
            corr_cer = editdistance.eval(
                preds_corrected[i], labels[i]) / max(len(labels[i]), 1)
            f.write(
                f"Original CER: {orig_cer:.4f}, Corrected CER: {corr_cer:.4f}\n")
            f.write("-" * 40 + "\n")
