...
def get_alphabet_from_data(data_list, data_path):
    ...
    try:
            ...
            try:
                ...
                with open(txt_path, 'r', encoding='utf-8') as txt_file:
                    ...
                    if line_num < 3:  # Debug first few
                        print(
                            f"DEBUG: {image_filename} -> {txt_filename}: '{ground_truth_text}'")
        ...
        print(
            f"DEBUG: Processed {processed_count} files, {error_count} errors")

        # Sort to ensure consistent ordering
        alphabet_str = ''.join(sorted(alphabet))
        print(
            f"DEBUG: Extracted alphabet ({len(alphabet_str)} chars): {alphabet_str[:100]}...")

        if len(alphabet_str) == 0:
            print("ERROR: No alphabet extracted! Check data paths and file structure")
            print(
                f"Expected structure: {data_path}/image_name.txt containing ground truth text")
    ...
    with torch.no_grad():
        for images, texts in evaluation_loader:
            ...
            loss, _, _ = compute_encoder_decoder_loss(
                model, images, texts, tokenizer, max_length)
            total_loss += loss.item() * batch_size

            # Generate predictions using configured method
            if batch_size == 1 and args.generation_method == 'beam_search':
            ...
            for pred_text, gt_text in zip(predicted_texts, texts):
                ...
                cer = editdistance.eval(
                    pred_text, gt_text) / max(len(gt_text), 1)
                total_cer += cer
                ...
                wer = editdistance.eval(
                    pred_words, gt_words) / max(len(gt_words), 1)
                total_wer += wer
...
def main():
    ...
    args.save_dir = os.path.join(
        args.out_dir, args.exp_name + '_encoder_decoder')
    ...
    test_input, test_output, test_lengths = tokenizer.encode_for_training(
        test_text)
    ...
    if model_vocab_size != vocab_info['vocab_size']:
        logger.error(
            f'MISMATCH: Model vocab size ({model_vocab_size}) != Tokenizer vocab size ({vocab_info["vocab_size"]})')
    ...
    while nb_iter < args.total_iter:
        ...
        if nb_iter % args.print_iter == 0:
            ...
            logger.info(
                f'Iter: {nb_iter} | LR: {current_lr:.6f} | Loss: {train_loss_avg:.4f}')
