...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,
        ...
        # no-tone alphabet size (108)
        self.base_head = nn.Linear(embed_dim, nb_cls)
        # six diacritic classes
        self.diac_head = nn.Linear(embed_dim, 6)
    ...
    def forward(self, x, mask_ratio=0.0, max_span_length=1, use_masking=False):
        ...
        # (T, B, C_base)
        logp_base = F.log_softmax(
            logits_base, dim=-1).permute(1, 0, 2)

        # Diacritic path
        logits_diac = self.diac_head(x)                           # (B, T, 6)

        return logp_base, logits_diac
