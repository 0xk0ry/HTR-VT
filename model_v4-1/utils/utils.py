...
def strip_tones(s):
    ...
    chars = unicodedata.normalize('NFD', s)
    return ''.join(ch for ch in chars if unicodedata.category(ch) != 'Mn')
...
def extract_diacritics(s, max_len):
    # returns a list of length max_len with -100 on pads
    base = unicodedata.normalize('NFD', s)
    diacs = []
    for ch in base:
        if unicodedata.category(ch) == 'Mn':
            # apply to previous char
            diacs[-1] = DIAC_MAP.get(ch, 0)
        else:
            diacs.append(0)
    # pad / truncate to max_len with -100
    L = len(diacs)
    if L < max_len:
        diacs += [-100]*(max_len-L)
    return diacs[:max_len]
...
class DiacriticConverter:
    ...
    def encode(self, label_str):
        # strip to NFD & extract diacritics
        diac_list = extract_diacritics(label_str, self.max_len)
        return torch.LongTensor(diac_list).to(device)
